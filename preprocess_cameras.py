...
import opt.utils.general as utils
...
def get_min_max_d(curx, cury, P_j, silhouette_j, P_0, Fj0, j):
    ...
    cur_l_1=Fj0 @ np.array([curx,cury,1.0]).astype(float)
    ...
    if relevant_matching_points_1.shape[1]==0:
        return (0.0,0.0)
    X = cv2.triangulatePoints(P_0, P_j, np.tile(np.array([curx, cury]).astype(float),
...
def get_all_mask_points(masks_dir):
    ...
    mask_paths = mask_paths[::2] #fixes some bug with statue dataset
    mask_points_all=[]
    mask_ims = []
...
def refine_visual_hull(masks, Ps, scale, center):
    ...
    GRID_SIZE=100
    MINIMAL_VIEWS=30 # Fitted for DTU, might need to change for different data.
