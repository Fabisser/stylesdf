...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        style_dir = '{0}/style'.format(self.instance_dir)
        style_paths = sorted(utils.glob_imgs(style_dir))
        ...
        for path in style_paths:
            style = rend_util.load_rgb(path)
            style = style.reshape(3, -1).transpose(1, 0)
            self.style_images.append(torch.from_numpy(style).float())
    ...
    def __getitem__(self, idx):
        ...
        ground_truth = {
            ...
            "style": self.style_images[idx]
        }

        if self.sampling_idx is not None:
            ground_truth["rgb"] = self.rgb_images[idx][self.sampling_idx, :]
            ground_truth["style"] = self.style_images[idx][self.sampling_idx, :]
