...
if __name__ == '__main__':
    ...
    output_cams = os.path.join(args.dense_folder, 'cameras')
    ...
    for i in range(num_images):
        ...
        P = np.vstack((W.T,T))
        P = np.vstack((P.T,np.array([0,0,0,1])))

        dict['world_mat_{}'.format(i)] = P
        
    np.savez(output_cams, **dict)  # data is a dict here
